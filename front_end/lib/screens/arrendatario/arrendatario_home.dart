...
import 'dart:convert';
import '../arrendatario/arrendatario_habitacion.dart';
...
class _ArrendatarioHomeState extends State<ArrendatarioHome> {
  ...
  List<dynamic> _filtrarHabitaciones(
    ...
    return habitaciones.where((habitacion) {
      ...
      final precioMax =
          (filtros['precioMax'] as num?)?.toDouble() ?? double.maxFinite;
      ...
      if (filtros['servicios'] != null &&
        ...
        final serviciosHabitacion = List<String>.from(
          habitacion['servicios'] ?? [],
        );
        final serviciosRequeridos = List<String>.from(filtros['servicios']);

        if (!serviciosRequeridos.every(
          (servicio) => serviciosHabitacion.any(
            (s) =>
                s.toString().toLowerCase() == servicio.toString().toLowerCase(),
          ),
        )) {
          return false;
        }
  ...
  Future<void> _mostrarFormularioReserva(
  ...
  Future<void> _procesarReserva(
    ...
    try {
      ...
      final datosReserva = {
        ...
        'datosBancarios': {'metodoPago': 'Por definir'},
      };
      ...
      if (response['success'] == true) {
        _mostrarMensaje(
          '¡Reserva enviada exitosamente! Número de contrato: $numeroContrato',
        );
      } else {
  ...
  void _mostrarMensaje(String mensaje) {
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text(mensaje), backgroundColor: Colors.green[700]),
    );
  }
      ...
      body: Padding(
        ...
        child: CustomScrollView(
          slivers: [
            ...
            SliverToBoxAdapter(child: WelcomeCard(usuario: widget.usuario)),
